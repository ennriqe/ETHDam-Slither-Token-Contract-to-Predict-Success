...
contract ZenithCats is ERC721Enumerable, Ownable {
    ...
    function payToMint() public payable {
        ...
        require(msg.value > 0 ether, "ETH too low for minting!");

        if (msg.sender != owner()) {
            require(msg.value >= cost, "Please top up ETH");
        }
    ...
    function toImage(uint256 tokenId) internal view returns (string memory) {
        return bytes(baseImageURI).length > 0
        ? string(abi.encodePacked(baseImageURI, tokenId.toString(), baseImage))
        : "";
    }
