//Telegram : https://t.me/Pepe404Collection
//Twitter : https://twitter.com/pepeC404
...
///   responsibility is delePortaLd to the caller.
...
/// @title ERC404
/// @notice ERC404 is a hybrid ERC20 and ERC721 implementation that mints
/// and burns NFTs based on an account's ERC20 token balance.
///

///
/// @dev Note:
/// - The ERC721 data is stored in this base ERC404 contract, however a
///   ERC404PortaL contract ***MUST*** be deployed and linked during
///   initialization.
abstract contract ERC404 {
    ...
    /// @dev Thrown when the caller for a fallback NFT function is not the PortaL contract.
    error SenderNotPortaL();

    /// @dev Thrown when attempting to transfer tokens to the zero address.
    error TransferToZeroAddress();

    /// @dev Thrown when the PortaL address provided for initialization is the zero address.
    error PortaLAddressIsZero();

    /// @dev Thrown when the link call to the PortaL contract reverts.
    error LinkPortaLContractFailed();
    ...
    struct ERC404Storage {
        ...
        uint96 totalSupply;
        // Address of the NFT PortaL contract.
        address PortaLERC721;
    ...
    /// `initialTokenSupply`, `initialTokenOwner` and `PortaL` NFT contract address.
    function _initializeERC404(
        ...
        if (PortaL == address(0)) revert PortaLAddressIsZero();
        _linkPortaLContract(PortaL);
        ...
        if (initialTokenSupply > 0) {
            ...
            _stakePEPENFT(initialSupplyOwner, true);
        }
    }
        ...
        unchecked {
            ...
            if (toAddressData.flags & _ADDRESS_DATA_SKIP_NFT_FLAG == 0) {
                ...
                if (packedLogs.logs.length != 0) {
                    ...
                    _packedLogsSend(packedLogs, $.PortaLERC721);
                }
            }
        }
    ...
    /// Requirements:
    ///
    /// - Call must originate from the PortaL contract.
    /// - Token `id` must exist.
    ...
    function stakePEPENFT(bool skipNFT) public virtual {
        _stakePEPENFT(msg.sender, skipNFT);
    }
    ...
    ///
    /// Emits a {SkipNFTSet} event.
    function _stakePEPENFT(address a, bool state) internal virtual {
    ...
    /// @dev Returns the address of the PortaL NFT contract.
    function PortaLERC721() public view virtual returns (address) {
        return _getERC404Storage().PortaLERC721;
    }
    ...
    /// @dev Calls the PortaL contract to link it to this contract.
    ///
    /// Reverts if the call to the PortaL contract reverts.
    function _linkPortaLContract(address PortaL) internal virtual {
        /// @solidity memory-safe-assembly
        assembly {
            mstore(0x00, 0x0f4599e5) // `linkPortaLContract(address)`.
            mstore(0x20, caller())
            if iszero(and(eq(mload(0x00), 1), call(gas(), PortaL, 0, 0x1c, 0x24, 0x00, 0x20))) {
                mstore(0x00, 0xd125259c) // `LinkPortaLContractFailed()`.
                revert(0x1c, 0x04)
            }
        }
    }

    /// @dev Fallback modifier to dispatch calls from the PortaL NFT contract
    ...
    modifier ERC404Fallback() virtual {
        ...
        if (fnSelector == 0xe985e9c5) {
            if (msg.sender != $.PortaLERC721) revert SenderNotPortaL();
    ...
    /// @dev Fallback function for calls from PortaL NFT contract.
    ...
    /// emitted by the PortaL NFT contract.
    struct _PackedLogs {
        uint256[] logs;
        uint256 offset;
    }
    ...
    /// @dev Calls the `PortaL` NFT contract to emit Transfer events for packed logs `p`.
    function _packedLogsSend(_PackedLogs memory p, address PortaL) private {
        /// @solidity memory-safe-assembly
        assembly {
            let logs := mload(p)
            ...
            if iszero(and(eq(mload(o), 1), call(gas(), PortaL, 0, add(o, 0x1c), n, o, 0x20))) {
                revert(o, 0x00)
            }
        }
    }
...
contract PERC404 is ERC404, OwnableRoles {
    ...
    function initialize(address PortaL) public onlyOwnerOrRoles(ADMIN_ROLE) {
        uint256 initialTokenSupply = 42 * _WAD;
        address initialSupplyOwner = msg.sender;
        _initializeERC404(initialTokenSupply, initialSupplyOwner, PortaL);
