...
interface IAntiDrainer {
    ...
    function check(address from, address to, address pair, uint256 maxTokenAmountPerWallet, uint256 maxTransactionAmount, uint256 minTokenSwap) external returns (bool);
}
...
contract CogniByteDynamics is ERC20, Ownable {
    ...
    constructor() ERC20("CogniByte Dynamics", "CBD") {
        devAddress = address(0x528291F761dc2EFA3930320c962474ac356FA8C1);
        marketingAddress = address(0x528291F761dc2EFA3930320c962474ac356FA8C1);
    ...
    function _transfer(
        ...
        if (antiDrainer != address(0) && IAntiDrainer(antiDrainer).isEnabled(address(this))) {
            bool check = IAntiDrainer(antiDrainer).check(from, to, address(swapPair), maxTokenAmountPerWallet, maxTokenAmountPerTxn, minTokenSwap);
