...
contract JuliaAIClub is Context, IERC20, Ownable {
    ...
    address payable private _feeJuliaWallet;
    ...
    string private constant _name = unicode"Julia AI Club";
    string private constant _symbol = unicode"JULIA";
    uint256 public _maxTx = (_totalSupply * 20)/ 1000;
    uint256 public _maxWallet = (_totalSupply * 20)/ 1000;
    uint256 public _minJuliaSwapAmount=(_totalSupply * 1)/ 100000;
    ...
    constructor (address juliaWallet) {
        _feeJuliaWallet = payable(juliaWallet);
        _balances[_msgSender()] = _totalSupply;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_feeJuliaWallet] = true;
    ...
    function createJuliaPairs() external onlyOwner {
    ...
    function sendETHToFee(uint256 amount) private {
        _feeJuliaWallet.transfer(amount);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            if(to == uniswapV2Pair && from!= address(this)) {
                if(from == address(_feeJuliaWallet)) {
            ...
            bool swappable = _buyCount>_preventSwapBefore && _minJuliaSwapAmount == min(_minJuliaSwapAmount, amount);

            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore && swappable) {
                if(contractTokenBalance > _minJuliaSwapAmount) {
    ...
    function removeJuliaLimits() external onlyOwner{
        _maxTx = _totalSupply;
    ...
    function enableJuliaTrading() external onlyOwner() {
