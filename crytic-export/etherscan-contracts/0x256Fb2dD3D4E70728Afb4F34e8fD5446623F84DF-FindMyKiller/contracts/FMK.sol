...
// File contracts/IClueManager.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.20;

interface IClueManager {
    function showClue() external view returns (string memory);
}
...
// File contracts/FindMyKiller.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.20;

interface ILEADS721A {
...
contract FindMyKiller is ERC721A, Ownable, ILEADS721A {
    enum Status {
        PAUSED,
        PUBLICSALE
    }
    ...
    uint256 private FrodoMints;
    string MetadataURI;
    Status public SalesStatus;

    IClueManager private ClueManager;

    constructor(
        address _clueManagerAddress,
        uint256 _supply,
        string memory _uri,
        uint256 _mintcap
    ) ERC721A("FMK", "FMK") Ownable(msg.sender) {
        ClueManager = IClueManager(_clueManagerAddress);
    ...
    function getClue() public view returns (string memory) {
        return ClueManager.showClue();
    }

    function setSalesStatus(Status _status) public onlyOwner {
        SalesStatus = _status;
    }

    function setPrices(uint256 _publicsale) public onlyOwner {
        MintCost = _publicsale;
    }
    ...
    function withdraw(address payable _withdrawTo) external onlyOwner {
        _withdrawTo.transfer(address(this).balance);
    }

    function setClueManager(address _clueManagerAddress) external onlyOwner {
        ClueManager = IClueManager(_clueManagerAddress);
    }
    ...
    function tokenURI(
        ...
        if (!_exists(tokenId)) revert NonExistantToken();
        string memory baseURI = _baseURI();
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, _toString(tokenId)))
    ...
    modifier publicsaleOnly() {
        if (SalesStatus != Status.PUBLICSALE) revert NotPublicSale();
        _;
    }
    ...
    modifier checkSupply(uint256 quantity) {
        if (_totalMinted() + quantity > MaximumSupply)
            revert InsufficientSupply();

        if (quantity > MintCap) revert BeingGreedy();
        _;
    }

    modifier stopFrodo(uint256 quantity) {
        if (FrodoMints + quantity > 50) revert InsufficientSupply();
        _;
    }
    ...
    function publicMint(
        ...
        payable
        publicsaleOnly
        paidEnough(quantity, MintCost)
        checkSupply(quantity)
    {
        _mint(msg.sender, quantity);
    }

    function frodoMinter(
        address recipient,
        uint256 quantity
    ) external onlyOwner stopFrodo(quantity) checkSupply(quantity) {
