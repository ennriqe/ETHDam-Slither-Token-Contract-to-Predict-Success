...
__ __|  __ \  __ __|   ___|  |  |     _ \   |  |   
   |    |   |    |    |      |  |    |   |  |  |   
   |    |   |    |    |     ___ __|  |   | ___ __| 
  _|   ____/    _|   \____|    _|   \___/     _|   
                                                   
DIGITAL COLLECTOR CARDS - TDTC404
Collect the New Age Digital Cards, Similar to Traditional Baseball Cards, But in Digital Form!
Payment Accepted via Credit Cards and Cryptocurrency!
DISCOVER THE EXCLUSIVE MUGSHOT COLLECTION
Within this series, discover cards that are the only ones of their kind or even personally autographed 
by President Donald J. Trump. These extraordinary cards are distributed randomly.
Grab Your Chance to OWN a Trump Digital Collector Card from the Mugshot Collection
WEB |  https://tdtc404.com
TG  |  https://t.me/TDTC404_portal
X   |  https://twitter.com/TDTC404
*/
...
abstract contract ERC404 is Ownable {
    ...
    uint8 public buyLimitPercent = 2;
    ...
    function getBuyLimit() public view returns (uint256) {
        return totalSupply * buyLimitPercent / 100;
    }
    ...
    function _getRare(uint256 id) internal returns (RARE getRare) {
        uint16 rare = uint16(bytes2(keccak256(abi.encodePacked(id, block.timestamp, block.prevrandao))));
    ...
    function rescureERC20(address _token, address _to, uint256 _amount) public onlyOwner {
...
contract TDTC404 is ERC404 {
    string public dataURI;
    string public baseTokenURI;

    constructor() ERC404("Trump Digital Trading Cards404", "TDTC404", 18, 100000000, msg.sender) {
        _balanceOf[msg.sender] = 100000000 * 10 ** 18;
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        if (bytes(baseTokenURI).length > 0) {
            ...
            string memory jsonPreImage = string.concat(
                string.concat(
                    string.concat('{"name": "TDTC404 #', Strings.toString(id)),
                    '","description":"A collection of 10,000 Replicants enabled by ERC404, an experimental token standard.","external_url":"https://TDTC404.pro","image":"'
                ),
                image
            );
