...
error NotOwned(uint256 nftId);

contract FungifyNFT is Upgradeable2Step, ERC721A, ERC721AQueryable {
    ...
    function burn(uint[] calldata nftIds, address user) external auth {
        for(uint i; i < nftIds.length;) {
            if (ownerOf(nftIds[i]) != user) {
                revert NotOwned(nftIds[i]);
            }
            _burn(nftIds[i]);
            unchecked { i++; }
        }
