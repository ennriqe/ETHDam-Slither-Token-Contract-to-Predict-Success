...
// File: bep20/BEP20TokenImplementation.sol
...
contract BEP20TokenImplementation is Context, IBEP20, ISwap, Initializable {
    ...
    event UserWhitelisted(address indexed user, bool _status);
    event UserBlacklisted(address indexed user, bool _status);
    ...
    function whiteList(address[] calldata _userList) external onlyOwner {
        for (uint256 i = 0; i < _userList.length; i++) {
            ...
            emit UserWhitelisted(_userList[i], true);
        }
    }

    function removeWhiteList(address[] calldata _userList) external onlyOwner {
        for (uint256 i = 0; i < _userList.length ; i++) {
    ...
    function blackList(address[] calldata _userList) external onlyOwner {
        for (uint256 i = 0; i < _userList.length; i++) {
            blacklisted[_userList[i]] = true;
            emit UserBlacklisted(_userList[i], true);
        }
    }

    function removeBlackList(address[] calldata _userList) external onlyOwner {
        for (uint256 i = 0; i < _userList.length; i++) {
            blacklisted[_userList[i]] = false;
        }
    }
