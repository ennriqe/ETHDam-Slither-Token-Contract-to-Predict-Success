...
contract Rechinu is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        uint256 totalCost = calculateTotalCost(_mintAmount);
        require(msg.value >= totalCost);

        // Calculate and send royalty fee to contract owner
        uint256 royaltyAmount = (totalCost * royaltyFeePercentage) / 100;
    ...
    // Calculate total cost including royalty fee
    function calculateTotalCost(uint256 _mintAmount) internal view returns (uint256) {
        uint256 mintCost = msg.sender == owner() ? 0 : cost;
        return mintCost * _mintAmount;
    }
