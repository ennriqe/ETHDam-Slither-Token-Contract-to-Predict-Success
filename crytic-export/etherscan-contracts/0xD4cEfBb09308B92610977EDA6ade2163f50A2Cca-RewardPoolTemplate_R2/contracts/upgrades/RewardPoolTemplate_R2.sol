...
contract RewardPoolTemplate_R2 is Initializable, ContextUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20Upgradeable, ERC165Upgradeable, IRewardPool {
    ...
    function initZeroRewardPayout(uint256 maxSupply, uint8 payoutType, uint64 fromBlock, uint64 toBlockExclusive, uint256 amount) external onlyMultiSig whenZeroSupply whenPaused override {
        ...
        // store initial supply value to correctly calculate rewards after some tokens are burned
    ...
    function _calcPendingRewards(uint256 balance, RewardState memory rewardState) internal view {
        ...
        // TODO: if this is ever used for token claim - CHECK that this is the supply you want instead of current totalSupply()!!!
        ...
        for (uint256 i = rewardState.activeRewardPayout; i < totalRewardPayouts; i++) {
            ...
            _calcPendingRewardsForPayout(balance, supply, rewardState, rewardPayout);
        }
    ...
    function _calcPendingRewardsForPayout(uint256 balance, uint256 supply, RewardState memory rewardState, RewardPayout memory currentPayout) internal view {
        ...
        if (currentPayout.payoutType == PAYOUT_TYPE_UNIFORM) {
            ...
            uint256 accountShare = 1e18 * balance / supply;
    ...
    // @dev intended as a migration method. initialSupply is normally set in initZeroRewardPayout
    function migrateSetInitialSupply(uint256 initSupply) external onlyOwner whenNotPaused {
